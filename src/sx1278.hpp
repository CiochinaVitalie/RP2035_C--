...
public:
    ...
    SX1278(ISPI *spi_context, IDelay *delay_context, IGPIO *gpio_context) : spi(spi_context), delay(delay_context), gpio(gpio_context)
    {
        spi->init();
        gpio->output_conf(PIN_NSS);
        gpio->output_conf(PIN_RST);
        gpio->output_conf(PIN_DIO0);
    }
